...
dic={}
user_info={}
if os.path.exists("details1.json"):
    pass
else:
    create=open("details1.json","w+")
    create.close()
...
if choose == 1:
    ...
    for check in pas:
        ...
        if(check=='@'or check=='$' or check=='_' or check=='#'):
            s+=1
    try:
        with open("details1.json","r") as output:
    ...
    if (l>=1 and u>=1 and s>=1 and d>=1 and l+s+u+d==len(pas)):
        ...
        if pas==pas1: 
            if  os.stat("details1.json").st_size==0:
                ...
                try:
                    ...
                    dic["Username"]=name
                    dic["Password"]=pas
                    dic["Profile"]=user_info
                    list.append(dic)
                    main_dict["user"]=list
                    with open("details1.json","r+") as file:
                        read_file= file.read()
                    ...
                    with open("details1.json","w") as f:
    ...
    with open("details1.json","r") as log_in_file:
